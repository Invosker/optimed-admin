...
import { Client } from "@/Views/doctorConfig/types/client";
...
export const useClientSearch = (raw: UseClientSearchParams) => {
  const { token } = useUser();
  ...
  if (search) {
    // simple OR search; backend may ignore unsupported operators but it's the project's pattern
    filterObj.$or = [
  ...
  return useQuery({
    queryKey: ["clients:search", { page, limit, filter: filterParam }],
    ...
    queryFn: async (): Promise<ApiResponse<Client[]>> =>
      await apiClient.get<ApiResponse<Client[]>>(CLIENTS_ENDPOINT, {
        params: {
          ...
          fields:
            "id,firstName,lastName,email,phone,identification,identificationType,address,description,dateOfBirth,isActive",
        },
      }),
